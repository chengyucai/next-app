...
const DataTables: React.FC<Props> = props => {
  ...
  const checkbox = (state: boolean, c_key: number, r_key: number) => {
    ...
    return c_key >= 0 && r_key >= 0 ? (
      <Int_cbox {...defult} {...data} />
    ) : (
      <Int_cbox {...defult} {...tital} />
    );
  };
  ...
  const First = (tital: string[]) => {
    const list = tital.map((data: string, i: number) => {
      return (
        <th
          className={cx("tital", type[i], `c_${i}`)}
          key={`${i}_First`}
          title={data}
        >
  ...
  const Data = (alldata: (number | string | boolean)[][]) => {
    const datalist = alldata.map(
      (list: (number | string | boolean)[], c_i: number) => {
        const tList = list.map(
          (data: number | string | boolean, r_i: number) => {
            return (
              <td className={cx(type[r_i], `c_${r_i}`)} key={`${r_i}_Data`}>
                {typeof data === "boolean" ? (
                  checkbox(data, c_i, r_i)
                ) : (
                  <span>{data}</span>
                )}
              </td>
